...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Future<bool> loginCorreo() async{
    var url = 'http://192.168.2.101:8085/login';
    var body = jsonEncode({ 'email': correoController.text,'password': contrasenaController.text});
    ...
    if (resp['success']!=null || resp['success']!='is not true') {
      ...
      if(token=="correcto"){
        ...
        User usuario=new User(userApi: resp['Api'],userEmail: correoController.text );
        setState(() {
          _usuario=usuario;
        });
        return true;
        ...
        onPressed: () async{
          ...
          if(value == true) {
            Navigator.push(
              context,
              MaterialPageRoute(
                builder: (context) => Home(user:_usuario),
              ),
            );
          }
        },
